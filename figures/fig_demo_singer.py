"""SINGER equation demo beside an msprime-simulated truth tree sequence.

This figure does not run SINGER and does not show an inferred ARG.
"""
...
# ── Compute source-equation quantities ─────────────────────────
t_max = 4 * Ne
...
fig.suptitle(
    f"SINGER mechanisms beside simulated truth ({ts.num_samples} haps, {ts.num_sites} sites)",
...
for allele_new, allele_lower, allele_upper, color, label in [
    (0, 0, 0, "#2166AC", "States (0,0,0)"),
    (1, 0, 0, "#B2182B", "States (1,0,0)"),
    (0, 1, 1, "#1B7837", "States (0,1,1)"),
    (1, 1, 1, "#E08214", "States (1,1,1)"),
]:
    probs = []
    for tau in tau_vals:
        p = emission_probability(
            allele_new, allele_lower, allele_upper,
            tau, 0.0, tau, theta_site,
        )
        probs.append(p)
...
for s_val, color in zip(s_vals, colors_c):
    densities = [psmc_transition_density(t, s_val, rho_bin) for t in t_vals]
...
ax.set_title(f"C. Continuous kernel ($\\rho={rho_bin}$ per bin)")
ax.legend(fontsize=7)
