...
# Plot nodes that actually have an incoming edge in this teaching projection.
child_ids = {child for _, _, _, child in edges}
sorted_nodes = sorted(
    (node for node in nodes if node["id"] in child_ids),
    key=lambda nd: -nd["time"],
)
...
sample_ys = [node_y[nd["id"]] for nd in sorted_nodes if nd["is_sample"]]
anc_ys = [node_y[nd["id"]] for nd in sorted_nodes if not nd["is_sample"]]
...
n_nodes = len(nodes)
n_edges = len(edges)
n_samp = sum(1 for nd in nodes if nd["is_sample"])
ax.text(0.02, 0.95,
