"""
Tests for watchgen.mini_lshmm -- the Li & Stephens HMM mini-implementation.

All functions are imported from watchgen.mini_lshmm; nothing is redefined here.
Test logic is adapted from tests/test_timepieces_lshmm.py.
"""
...
from watchgen.mini_lshmm import (
    ...
    path_loglik_hap,
    diploid_transition_prob,
    emission_matrix_diploid,
    genotype_comparison_index,
    build_genotype_matrix,
    forward_diploid,
    viterbi_diploid,
    backwards_viterbi_diploid,
    get_phased_path,
)
...
class TestEmissionMatrixHaploid:
    ...
    def test_multiallelic_emission_sums(self):
        """For a site with a alleles, match + (a-1)*mismatch = 1."""
        mu = 0.02
        for a in [2, 3, 4]:
...
class TestEstimateMutationProbability:
    ...
    def test_decreases_with_n(self):
        """mu should decrease as n increases."""
...
class TestForwardSteps:
    ...
    def test_naive_vs_fast(self):
        ...
        assert np.allclose(alpha_naive, alpha_fast)
...
class TestForwardLSHaploid:
    ...
    def test_all_match_higher_ll(self):
        ...
        s_match = np.array([0, 0, 0])
        s_bad = np.array([0, 1, 0])
        mu = 0.01
...
class TestDiploidTransitionProb:
    ...
    def test_factors_as_product(self):
        """Diploid transition should factor as product of two haploid."""
        n, r = 5, 0.15
...
class TestEmissionMatrixDiploid:
    ...
    def test_both_hom_match(self):
        ...
        expected = (1 - mu) ** 2
        assert np.isclose(e[0, 4], expected)

    def test_both_hom_mismatch(self):
        mu = 0.01
        e = emission_matrix_diploid(mu, 1, np.array([2]))
        expected = mu ** 2
        assert np.isclose(e[0, 0], expected)

    def test_both_het(self):
        mu = 0.01
        e = emission_matrix_diploid(mu, 1, np.array([2]))
        expected = (1 - mu)**2 + mu**2
        assert np.isclose(e[0, 7], expected)

    def test_ref_hom_obs_het(self):
        mu = 0.01
        e = emission_matrix_diploid(mu, 1, np.array([2]))
        expected = 2 * mu * (1 - mu)
        assert np.isclose(e[0, 1], expected)

    def test_ref_het_obs_hom(self):
        mu = 0.01
        e = emission_matrix_diploid(mu, 1, np.array([2]))
        expected = mu * (1 - mu)
        assert np.isclose(e[0, 2], expected)
...
class TestGetPhasedPath:
    def test_roundtrip(self):
        ...
        n = 5
        flat_path = np.array([0, 6, 12, 18, 24])
...
class TestForwardBackwardIntegration:
    ...
    def test_forward_backward_complete_pipeline(self):
        ...
        # Forward LL should be >= Viterbi LL
...
class TestDiploidForwardAlgorithm:
    ...
    def test_normalized_sums_to_one(self):
        ...
        F, c, ll = forward_diploid(n, m, G, query_gt, e_dip, r, norm=True)
        for l in range(m):
            assert np.isclose(F[l].sum(), 1.0, atol=1e-8), \
                f"Forward probs do not sum to 1 at site {l}"

    def test_norm_and_raw_same_ll(self):
        np.random.seed(42)
        n = 3
        m = 8
        H = np.random.binomial(1, 0.3, size=(m, n))
...
class TestDiploidViterbi:
    """Test the diploid Viterbi algorithm."""
    ...
    def test_viterbi_path_length(self):
        ...
        assert len(flat_path) == m

    def test_viterbi_ll_finite(self):
        np.random.seed(42)
        n = 4
        m = 15
        H = np.random.binomial(1, 0.3, size=(m, n))
    ...
    def test_phased_paths_valid(self):
        np.random.seed(42)
        n = 4
        m = 15
        H = np.random.binomial(1, 0.3, size=(m, n))
        ...
        assert np.all(path1 >= 0)
        assert np.all(path1 < n)
        assert np.all(path2 >= 0)
        assert np.all(path2 < n)
