"""
Tests for Python code blocks from the dadi timepiece RST documentation.

The dadi chapter teaches the diffusion approximation approach to computing
the site frequency spectrum. Since the RST code blocks use the external
dadi package, we re-implement the core mathematical concepts from scratch
and verify their correctness via structural properties and known results.

Covers:
- diffusion_equation.rst: equilibrium SFS density, non-uniform grids
- numerical_integration.rst: Crank-Nicolson PDE integration, SFS extraction
- demographic_inference.rst: Poisson/multinomial likelihoods, optimal scaling
"""
...
# ---------------------------------------------------------------------------
# Re-defined functions from docs/timepieces/dadi/diffusion_equation.rst
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Re-defined functions from docs/timepieces/dadi/numerical_integration.rst
# ---------------------------------------------------------------------------

def crank_nicolson_1d(phi, xx, T, nu=1.0, theta=0.0, n_steps=100):
    """Integrate the 1D diffusion equation using the Crank-Nicolson scheme.
    ...
    for _ in range(n_steps):
        ...
        for i in range(1, n - 1):
            ...
            # Diffusion coefficient: V(x) = x(1-x)/(2*nu)
            V = xx[i] * (1 - xx[i]) / (2.0 * nu)

            # Second derivative of V*phi using finite differences
            V_r = 0.5 * (xx[i] + xx[i + 1]) * (1 - 0.5 * (xx[i] + xx[i + 1])) / (2.0 * nu)
            V_l = 0.5 * (xx[i] + xx[i - 1]) * (1 - 0.5 * (xx[i] + xx[i - 1])) / (2.0 * nu)

            d2 = (V_r * (phi[i + 1] - phi[i]) / dx_r
                   - V_l * (phi[i] - phi[i - 1]) / dx_l) / dx_avg

            phi_new[i] = phi[i] + dt * d2

        # Mutation injection at the singleton bin
        if theta > 0 and n > 2:
            phi_new[1] += theta / (2.0 * xx[1]) * dt
...
# ---------------------------------------------------------------------------
# Re-defined functions from docs/timepieces/dadi/demographic_inference.rst
# ---------------------------------------------------------------------------
...
class TestCrankNicolson1d:
    ...
    def test_expansion_spreads_density(self):
        ...
        # Check that interior mass is relatively higher
